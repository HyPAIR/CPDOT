...
public:
  ...
  virtual bool SolveFm(double w_inf, const std::vector<Constraints> &profile, const std::vector<FullStates> &guess, std::vector<FullStates> &result, double &infeasibility, double& solve_time, 
  const std::vector<std::vector<std::vector<std::vector<double>>>>& corridor_cons, std::vector<double>& height_cons) {
    return false;
  }
