...
void CalCorridors(
  ...
  std::vector<std::vector<std::vector<std::vector<double>>>>& hPolys_sets,
  std::vector<std::vector<std::vector<math::Vec2d>>>& corri_vertices_sets) {
    ...
    std::cout << std::endl;
    for (int i = 0; i < combinations_new.size(); i++) {
      // optional: local boxes
      for (int j = 0; j < paths_sets[i][0].path.size(); j++) {
          math::AABox2d box;
          if (!env->GenerateCorridorBox(0.0, paths_sets[i][combinations_new[i]].path[j].x(), paths_sets[i][combinations_new[i]].path[j].y(), 0.3, box)) {
