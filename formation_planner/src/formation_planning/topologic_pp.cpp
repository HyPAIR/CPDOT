        ...
        for (int case_num = 0; case_num < 100; case_num++) {
          ...
          #pragma omp parallel 
          {
            ...
            for (int i = 0; i < iter_outer_max; i++) {
              ...
              CalCorridors(paths_sets, combinations_new[i], env, corridors_sets, keyPts, hPolys_sets, corri_vertices_sets);
              ...
              if(!planner_->Plan_fm(
                ...
                solve_time_set, show_cr, corridors_sets, hPolys_sets, path_pub_set, iteration_num_inner, cost, solve_success,
                e_max, e_avg, avg, std)) {
                #pragma omp critical 
                {
                  ROS_ERROR("re-plan trajectory optimization failed!");
