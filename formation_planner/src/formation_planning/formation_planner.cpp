...
bool FormationPlanner::Plan_fm(
  ...
  const std::vector<std::vector<std::vector<std::vector<double>>>>& hPolys_sets,
  ...
  for (int ind = 0; ind < guess.size(); ind++) {
    ...
    for(size_t i = 0; i < guess[ind].states.size(); i++) {
      ...
      // if (show_cr && ind == 0) {
      //   iris::IRISOptions options;
      //   iris_problem.setSeedPoint(Eigen::Vector2d(guess[ind].states[i].x, guess[ind].states[i].y));
      //   iris::IRISRegion region = iris::inflate_region(iris_problem, options);
      //   auto points = region.polyhedron.generatorPoints();
      //   std::vector<math::Vec2d> points_;
      //   for (const auto& pts : points) {
      //     math::Vec2d pts_temp(pts[0], pts[1]);
      ...
      //   color.set_alpha(0.05);
      //   // visualization::PlotPolygon(math::Polygon2d(math::Box2d(box)), 0.05, color, i, "Corridor " + std::to_string(i));
      //   visualization::PlotPolygon(math::Polygon2d({points_}), 0.05, color, 0, "Convex Region" + std::to_string(0));
      // // } 
      // }
    }
  }
  ...
  warm_start < 15) {
    ...
    if (result[0].tf < 150 && warm_start != 0) {
      for (int j = 0; j < result.size(); j++) {
        writeTrajectoryToYAML(result[j], "/home/weijian/CPDOT/src/formation_planner/traj_result/flexible_formation/" 
        + std::to_string(num_robot) + "/traj_" + std::to_string(num_robot) + std::to_string(j) + std::to_string(1000) +".yaml");
      }  
    }
    // generate good initial guess
    if (warm_start < 5) {
      if(!problem_->SolveFm(config_->opti_w_penalty0, hPolys_sets, constraints, guess, result, infeasibility, solve_time, height_cons_set)) {
