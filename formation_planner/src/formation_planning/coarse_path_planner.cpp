...
//
#include "formation_planner/coarse_path_planner.h"
...
bool CoarsePathPlanner::Plan(math::Pose start, math::Pose goal, std::vector<math::Pose> &result, const std::vector<std::vector<std::vector<double>>> hyperparam_set) {
  ...
  while(!open_pq_.empty()) {
    ...
    // 检查是否可以一步到达
    if(should_check && CheckOneshotPath(node, goal_node, oneshot_path, hyperparam_set)) {
    ...
    for(int i = 0; i < next_node_num; i++) {
      Node3d next_node;
      if(!ExpandNextNode(node, i, next_node, hyperparam_set)) {
        continue;
      }
...
bool CoarsePathPlanner::ExpandNextNode(const Node3d &node, int next_index, Node3d &next_node, const std::vector<std::vector<std::vector<double>>> hyperparam_set) {
  double steering;
  ...
  if(!env_->CheckHomotopyConstraints(0.0, path[path.size() - 1], hyperparam_set)) {
    return false;
  }
...
bool CoarsePathPlanner::CheckOneshotPath(const Node3d &node, const Node3d &goal, std::vector<math::Pose> &result, const std::vector<std::vector<std::vector<double>>> hyperparam_set) {
  ...
  if(!env_->CheckHomotopyConstraints(0.0, result[result.size() - 1], hyperparam_set)) {
    return false;
  }
  return true;
}
