    ...
    if (FormationFeasible(vertice_initial_set, robot_pos_set)) {
        for (int taut_num = 3; taut_num < robot_pos_set.size() + 1; taut_num++) {
            ...
            for (int i = 0; i < taut_combinations.size(); i++) {
                ...
                std::vector<int> taut_sub_set;
                for (int j = 0; j < taut_combinations[i].size(); j++) {
                    ij_index_set.push_back(taut_combinations[i][j] - 1);
                    taut_sub_set.push_back(taut_combinations[i][j] - 1);
                }
                if (taut_sub_set[0] == 3 && taut_sub_set[1] == 4 && taut_sub_set[2] == 7) {
                    std::cout << "!!" << std::endl;
                }
                for (int j = 0; j < ij_index_set.size(); j++ ) {
                    polygon.push_back({robot_pos_set[ij_index_set[j]][0], robot_pos_set[ij_index_set[j]][1]});
                }
                ...
                if (CheckRankEquality(A1, b1)) {
                    ...
                    if (CheckFeasibility(x, A2, b2, xi1, yi1, xvi1, yvi1, zr, zo)) {
                        if (CheckForceClosure(x, polygon)) {
                            ...
                            taut_set.push_back(taut_sub_set);
                        }
                    }
                }
                else {
                    std::cout << "Rank equality not satisfied!" << std::endl;
                }
            }
        }
        return true;
...
bool ForwardKinematics::isPointInPolygon(const std::vector<Point>& polygon, const Point& point) {
    int intersectCount = 0;
    int n = polygon.size();

    Point origin = {point.x, point.y};

    for (int i = 0; i < n; ++i) {
        Point v1 = polygon[i];
        Point v2 = polygon[(i + 1) % n];
        if ((v1.y <= origin.y && v2.y <= origin.y) || (v1.y > origin.y && v2.y > origin.y)) {
            continue;
        }
        double intersectX = (v2.x - v1.x) * (origin.y - v1.y) / (v2.y - v1.y) + v1.x;
...
bool ForwardKinematics::CheckForceClosure(
    ...
    std::vector<Point> polygon_robot;
    Point ro({x(0), x(1)});
    for (int i = 0; i < polygon.size(); i++) {
        polygon_robot.push_back({polygon[i](0), polygon[i](1)});
    }
    if(!isPointInPolygon(polygon_robot, ro)) {
        std::cout << "Force closure not satisfied!" << std::endl;
        return false;
    }
    else {
        return true;
