...
// //
// #include "formation_planner/formation_planner_ros.h"
...
// PLUGINLIB_EXPORT_CLASS(formation_planner::FormationPlannerROS, nav_core::BaseLocalPlanner)

// namespace formation_planner {

// FormationPlannerROS::FormationPlannerROS() : costmap_ros_(NULL), tf_(NULL), initialized_(false) {}

// FormationPlannerROS::FormationPlannerROS(std::string name, tf2_ros::Buffer *tf,
//                                          costmap_2d::Costmap2DROS *costmap_ros)
//     : costmap_ros_(NULL), tf_(NULL), initialized_(false) {
//   initialize(name, tf, costmap_ros);
// }

// FormationPlannerROS::~FormationPlannerROS() {}
// void DrawTrajectoryRvizRos(const FullStates sol_traj,std::shared_ptr<formation_planner::PlannerConfig> config,
...
//   visualization::Trigger();
// }
// void FormationPlannerROS::ReadParameters(const ros::NodeHandle &nh) {
...
// void FormationPlannerROS::initialize(std::string name, tf2_ros::Buffer *tf,
//                                      costmap_2d::Costmap2DROS *costmap_ros) {
//   if (!initialized_) {
//     tf_ = tf;
...
//     planner_config_ = std::make_shared<formation_planner::PlannerConfig>();
//     planner_config_->vehicle.InitializeDiscs();

//     env_ = std::make_shared<formation_planner::Environment>(planner_config_);
//     planner_ = std::make_shared<formation_planner::FormationPlanner>(planner_config_, env_);

//     ros::NodeHandle nh("~/" + name_);
//     ReadParameters(nh);

//     visualization::Init(nh, config_.global_frame, "formation_markers");
...
// bool FormationPlannerROS::setPlan(const std::vector<geometry_msgs::PoseStamped> &orig_global_plan) {
//   if (!initialized_) {
...
// bool FormationPlannerROS::computeVelocityCommands(geometry_msgs::Twist &cmd_vel) {
...
// bool FormationPlannerROS::isGoalReached() {
//   if (!initialized_) {
//     ROS_ERROR("This planner has not been initialized");
//     return false;
//   }
...
PLUGINLIB_EXPORT_CLASS(formation_planner::FormationPlannerROS, nav_core::BaseGlobalPlanner)

namespace formation_planner {

FormationPlannerROS::FormationPlannerROS() : costmap_ros_(NULL), tf_(NULL), initialized_(false) {}

FormationPlannerROS::FormationPlannerROS(std::string name,
                                         costmap_2d::Costmap2DROS *costmap_ros)
    : costmap_ros_(NULL), tf_(NULL), initialized_(false) {
...
// Take note that tf::TransformListener* has been changed to tf2_ros::Buffer* in ROS Noetic
void FormationPlannerROS::initialize(std::string name,
                                     costmap_2d::Costmap2DROS *costmap_ros) {
  if (!initialized_) {
    costmap_ros_ = costmap_ros;
    name_ = name;
...
//     writeVectorToYAML(new_vector, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/costmap.yaml");
//   }
  iris::IRISProblem iris_problem(2);
  geometry_msgs::PoseStamped current_pose;
  start_state_.x = current_pose.pose.position.x;
...
// bool FormationPlannerROS::makePlan(const geometry_msgs::PoseStamped& start, 
...
bool FormationPlannerROS::makePlan(const geometry_msgs::PoseStamped& start_, 
