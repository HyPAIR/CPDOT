...
int main(int argc, char* argv[])
{
    ...
    double height = 1.0;
    std::vector<std::vector<double>> vertice_initial_set, vertice_initial_set_;
    generateRegularPolygon(0.0, 0.0, 0.9, num_robot, vertice_initial_set);
    // std::vector<std::vector<double>> vertice_initial_set = {
    //     {-0.32, -0.42},
    //     {0.80, -0.38},
    //     {0.75, 0.71}, 
    ...
    for (int i = vertice_initial_set.size() - 1; i < 2 * vertice_initial_set.size() - 1; i++) {
        vertice_initial_set_.push_back(vertice_initial_set[i % vertice_initial_set.size()]);
    }
    while (ros::ok()) {
        fk_test.SolveFk(vertice_initial_set_, robot_pos_set, pos_3d, pos_2d, taut_set, height);
        break;
        ros::spinOnce();
        rate.sleep();	
	}
    // writeVectorToYAML(y_, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/flexible_formation/height.yaml");
    plt::clf();
    plt::xlabel("x/m");
    plt::ylabel("y/m");
    int index = 1;
