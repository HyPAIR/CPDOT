...
class FormationControllerNode(Node): 
    def __init__(self):
        ...
        # Kp_x : 0.4 # 0.4 #4.0
        # Kp_y : 0.4 #0.5 #10.5
        # Kp_phi : 0.3 # 0.575 #1.575
        # Kp_x : 0.8 # 0.4 #4.0
        # Kp_y : 0.5 #0.5 #10.5
        # Kp_phi : 0.2 # 0.575 #1.575
        self.path_array = []
        ...
        self.create_subscription(Odometry, f'{self.robot_names[0]}/odom', self.odom_callback1, qos_profile)
    ...
    def wait_for_odom(self, executor):
        ...
        while not (self.odom_received_1):
        # while i < 30:
            try:
                ...
                break
        path_array_ = [self.ObtainPath(str(self.robot_index))]
    ...
    def ObtainPath(self, index):
        path = []
        with open('/home/weijian/Downloads/traj_icra_complex23' + index + '0.yaml', 'r') as file:
    ...
    def run(self, executor):
        ...
        # while rclpy.ok() :
            ...
            for i in range(len(self.robot_names)):
                ...
                # print(np.hypot(target_points[i][1] - target_poses[i].position.y, target_points[i][0] - target_poses[i].position.x))
                if (np.hypot(target_points[i][1] - target_poses[i].position.y, target_points[i][0] - target_poses[i].position.x) < 1e-2):
                    target_angles[i] = 0.0
